...
export const ProductGrid: React.FC = () => {
    ...
    return products
      .filter((product) => {
        ...
        if (searchQuery.trim()) {
          ...
          if (!matchesTitle && !matchesBrand && !matchesCategory && !matchesDesc && !matchesDeal) return false;
        }

        if (minRating > 0 && product.rating < minRating) return false;
        if (product.price > maxPrice) return false;
        if (onlyPrime && !product.isPrime) return false;
        if (onlyInStock && product.stock <= 0) return false;
        if (selectedBrandsSet.size > 0 && !selectedBrandsSet.has(product.brand)) return false;
  ...
  }, [products, selectedCategory, searchQuery, minRating, maxPrice, onlyPrime, onlyInStock, selectedBrandsSet, sortBy]);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-6">
        ...
        <div className="flex flex-wrap items-center gap-2 py-3">
          ...
          {selectedCategory !== 'All' && (
            <span className="inline-flex items-center gap-1 bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 text-xs font-bold px-2.5 py-1 rounded-full">
              ...
              <button onClick={() => setSelectedCategory('All')} aria-label="Clear category filter" className="hover:text-black">×</button>
            </span>
          )}
          ...
          {searchQuery && (
            <span className="inline-flex items-center gap-1 bg-blue-100 dark:bg-blue-950/60 text-blue-800 dark:text-blue-300 text-xs font-bold px-2.5 py-1 rounded-full">
              Search: {searchQuery}
              <button onClick={() => setSearchQuery('')} aria-label="Clear search filter" className="hover:text-black">×</button>
            </span>
          )}
          ...
          {minRating > 0 && (
            <span className="inline-flex items-center gap-1 bg-yellow-100 dark:bg-yellow-950/60 text-yellow-800 dark:text-yellow-300 text-xs font-bold px-2.5 py-1 rounded-full">
              {minRating}★ & Up
              <button onClick={() => setMinRating(0)} aria-label="Clear minimum rating filter" className="hover:text-black">×</button>
            </span>
          )}
          ...
          {onlyPrime && (
            <span className="inline-flex items-center gap-1 bg-blue-100 dark:bg-blue-950/60 text-blue-800 dark:text-blue-300 text-xs font-bold px-2.5 py-1 rounded-full">
              Prime Only
              <button onClick={() => setOnlyPrime(false)} aria-label="Clear prime filter" className="hover:text-black">×</button>
            </span>
          )}
          ...
          {selectedBrands.map((b) => (
            <span key={b} className="inline-flex items-center gap-1 bg-gray-100 dark:bg-slate-800 text-gray-800 dark:text-gray-200 text-xs font-bold px-2.5 py-1 rounded-full">
              Brand: {b}
              <button onClick={() => handleToggleBrand(b)} aria-label={`Remove brand filter for ${b}`} className="hover:text-black">×</button>
            </span>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 pt-4">
        <FilterSidebar
          ...
          allBrands={allBrands}
          selectedBrandsSet={selectedBrandsSet}
