  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/50 backdrop-blur-xs flex justify-end animate-in fade-in duration-200">
      ...
      >
        ...
        {/* Action Bar */}
        <div className="px-5 py-2.5 bg-gray-50 dark:bg-slate-850 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between text-xs">
            ...
            notifications.map((notif) => (
              <div
                ...
                className={`p-3.5 rounded-xl border transition-all cursor-pointer space-y-1 ${
                  !notif.read
                    ? 'bg-amber-50/60 dark:bg-amber-950/20 border-amber-300 dark:border-amber-900/60 shadow-xs'
                    : 'bg-white dark:bg-slate-850 border-gray-100 dark:border-slate-800 hover:bg-gray-50'
                }`}
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-full bg-white dark:bg-slate-800 flex items-center justify-center shadow-xs shrink-0 border border-gray-200 dark:border-slate-700">
                      {getIcon(notif.type)}
                    </div>
        ...
        <div className="p-4 border-t border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-850 space-y-2">
          <button
